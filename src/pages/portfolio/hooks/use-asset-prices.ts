...
export const useAssetPrices = (assets: Metadata[] = []) => {
  ...
  const assetPrices: Record<string, AssetPrice> = useMemo(() => {
    ...
    const usdcPairs = data.filter(summary => summary.quoteAsset.symbol === 'USDC' || summary.quoteAsset.symbol === 'usdc');
    ...
    const stablePairs = data.filter(summary => !usdcPairs.includes(summary) && (
    ...
    const usdyDirectPair = data.find(summary => summary.baseAsset.symbol === 'USDY' && summary.quoteAsset.symbol === 'USDC');
